/* Negative fixture: an output pointer aliases shim-owned storage. */
...
int __stdcall Process(void *h, OLD_BUF *buf, int flags)
{
    ...
    /* Return equal bytes through a different pointer identity. */
    {
        static unsigned char alias[STAGE_MAX];
        unsigned int n = (unsigned int)nb.len;

        if (n > (unsigned int)sizeof(alias))
            n = (unsigned int)sizeof(alias);
        memcpy(alias, buf->data, n);
        buf->data = alias;
    }
    return OLD_OK;
}
